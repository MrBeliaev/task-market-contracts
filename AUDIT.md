...
**Dependencies**: OpenZeppelin Contracts v5 (`-upgradeable`: AccessControl, UUPS, Initializable; `ReentrancyGuard` from the base package, ERC-7201 namespaced storage)
**Date**: 2026-06-22
...
The contract has no `receive()` or fallback, so direct ETH transfers revert. This is the correct and intended behavior.
...
Source: `npx hardhat test --gas-stats`, Hardhat 3's built-in statistics across all 91 test calls.
...
- `confirmCompletion` max (88 955) is the second confirmation triggering `_releaseFunds`, which writes `pendingFees` and `pendingWithdrawals`
...
- Struct packing saves gas: `Task` fits in 4 slots instead of 8, about 80 000 gas saved per `createTask` versus an unpacked layout
...
| Reentrancy guard storage-safety                              | ✅     | `ReentrancyGuard` (OZ v5) uses ERC-7201 namespaced storage at a deterministic slot, with no collision with sequential state; initial slot value `0` is safe (guard checks `== ENTERED(2)`, not `!= NOT_ENTERED(1)`); `unsafeAllow: ["constructor"]` passed to `deployProxy`/`upgradeProxy` since OZ's plugin flag is not automatically suppressed by `@custom:stateless` |
...
| On-chain admin check via `hasRole`, so no off-chain whitelist bypass is possible               | ✅     |
| Initial `feeBps` and `feeRecipient` values emitted via events in `initialize`                  | ✅     |
| `feeBps` snapshot captured at task creation protects the executor from platform fee changes    | ✅     |
...
| Echidna (property fuzzing)   | 2.3.2     | All 6 invariants passing across 50 000 sequences           |
